...
Serial::waitReadable() {
	...
	return serial::Serial::SerialImpl::waitReadable(timeout.read_timeout_constant);
}

void
Serial::waitByteTimes(size_t count) {
	serial::Serial::SerialImpl::waitByteTimes(count);
}
...
size_t
Serial::readline(string &buffer, size_t size, const string &eol) {
	ScopedReadLock lock(this->pimpl_);
	size_t         eol_len     = eol.length();
	auto           *buffer_    = static_cast<uint8_t *>
...
string
Serial::readline(size_t size, const string &eol) {
	std::string buffer;
...
vector<string>
Serial::readlines(size_t size, const string &eol) {
	ScopedReadLock           lock(this->pimpl_);
	...
	while (read_so_far < size) {
		...
		if (bytes_read == 0) {
			if (start_of_line != read_so_far) {
				lines.emplace_back(reinterpret_cast<const char *> (buffer_ + start_of_line),
...
void Serial::sendBreak(int duration) {
	serial::Serial::SerialImpl::sendBreak(duration);
}
