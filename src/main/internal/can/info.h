...
namespace autolabor {
    namespace can {
        ...
        public:
            ...
            static_assert(std::is_same<_data_t, union_no_data>::value || std::is_same<_data_t, union_with_data>::value,
            ...
            constexpr static uint8_t bytes[]{
                static_cast<uint8_t>(network << 6u) | (data_field ? (1u << 5u) : 0u) | (property << 2u) | (node_type >> 4u),
                static_cast<uint8_t>(node_type << 4u) | node_index,
                type};
            
            inline static bool match(const data_t &msg) {
                const auto info = msg.data.info();
                return info.node_type() == node_type && info.node_index() == node_index && msg.data.type == type;
            }
        };
