...
namespace autolabor {
	namespace pm1 {
		...
		namespace ecu {
			...
			// 目标速度
			using target_speed        = can_pack_info<msg, 0, 0, id, 0x1>;
			// 当前速度
			using current_speed_tx    = can_pack_info<sgn, 0, 0, id, 0x5>;
			using current_speed_rx    = can_pack_info<msg, 0, 0, id, 0x5>;
			// 当前编码器读数
			using current_position_tx = can_pack_info<sgn, 0, 0, id, 0x6>;
			using current_position_rx = can_pack_info<msg, 0, 0, id, 0x6>;
			// 超时时间
			using timeout             = can_pack_info<msg, 0, 0, id, 0xa>;
		}
		...
		namespace tcu {
			...
			// 目标角度
			using target_position     = can_pack_info<msg, 0, 0, id, 0x1>;
			// 当前角度
			using current_position_tx = can_pack_info<sgn, 0, 0, id, 0x3>;
			using current_position_rx = can_pack_info<msg, 0, 0, id, 0x3>;
			// 当前速度
			using current_speed_tx    = can_pack_info<sgn, 0, 0, id, 0x5>;
			using current_speed_rx    = can_pack_info<msg, 0, 0, id, 0x5>;
		}
		...
		void fill_info(msg_union<typename info_t::data_t> &msg) {
			msg_union<uint16_t> info{0, 0};
